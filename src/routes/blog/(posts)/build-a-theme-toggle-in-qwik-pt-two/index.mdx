...
Qwik provides documentation on how to set up theme management in the [cookbook section](https://qwik.dev/docs/cookbook/theme-management/) of their website. However, I found it unsatisfactory for my needs, which is why I've created this post. That said, this section of the post is heavily inspired by the Qwik documentation.
...
Qwik uses [Vite](https://vitejs.dev/) as a build tool. We can use a Vite specific feature to import the raw text of a code file and pass that to the script tag. We'll see how to do that in a moment, but let's first create some code to pass.

Under the `src` directory, create a new directory called `utils`. In this directory, create a new file called `setInitialTheme.js`. We use a JS file here because it is going to be run directly on the browser while it loads, and the browser does not understand un-transpiled TypeScript. Add the following code to the file:
...
You will immediately notice some duplication from the `themeHelpers.ts` file. I initially tried to export everything from this file and import into the dropdown files, but this caused errors due to the use of `document`, `window`, and `localStorage`, which are all only available in the browser. In the server environment these are undefined and cause errors when imported. So the duplication is a necessary evil. With that out of the way, let's break down what this code does.
...
Now we need to import this file into the `root.tsx` file. Open that file and add the following import.

```tsx
import setInitialThemeAsString from "./utils/setInitialTheme?raw"
```

The `?raw` at the end of the import tells Vite to import the raw text of the file. Now we can add the script tag to the file.
...
I don't know about you, but I for one like this a whole lot better than writing code within a string. And it works! If you refresh the page, you should see the theme you selected last time you visited. If you change your system theme, the website should update to reflect that. And if you select a theme in the dropdown, the website should update to reflect that as well. But there is still one problem. On refresh, the icon shown in the dropdown button is always the system icon. Thankfully this is an easy fix, and allows us to see one last feature of Qwik.
...
One possible way to fix this could be to use a backend database to store user preferences and load them on the server in the pre-rendering process. If you try this out, I would be interested to hear how it works!
