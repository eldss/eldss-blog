...
In this blog post, we'll explore how to create a website to host a personal blog using the [Qwik](https://qwik.dev/) frontend framework. Qwik is designed for instant-loading and high performance, making it an excellent choice for building modern web applications and an interesting framework to expore. We will only explore the basics here, but you can use this as a starting point to build out a full-featured blog site.
...
You will be prompted to choose a directory to install the files in, a template for the project and to enter a project name. We'll put the project in `./my-blog` and use the "Empty App" template. Select "Yes" to install pnpm dependencies and to initialize a new git repository.
...
In this component, we are creating a simple header and nav bar that will be present on all pages. We are using the `component$` function from Qwik to define the component. You will see the dollar sign a lot in Qwik. You can think of it as indicating a lazy loading boundary, which allows Qwik to load the component only when it is needed. You can read more about defining components in the [Qwik documentation](https://qwik.dev/docs/components/overview/).
...
There are many possible ways to structure a blog in Qwik, but for simplicity, we will create our posts using [MDX](https://mdxjs.com/), which is a Markdown language that allows you to import and use JSX components. Qwik supports MDX [out of the box](https://qwik.dev/docs/guides/mdx/), and comiles it to Qwik components. This makes it a great way to write content heavy web pages like blog posts. It may not be ideal for non-technical writers who are not familiar with Markdown, or for blog sites that have authors as users, like Medium, which require more scalable solutions. But it works great for personal blogs. Especially if you are a developer who is comfortable with Markdown and working in frontend frameworks.
...
`formatDate` is a simple helper function to create a well formatted date string. It is defined by the code below, but feel free to adjust it to your tastes. You can add this to a new file in a `utils` directory, or in the same file as the component.
...
When you are ready to deploy, Qwik has several ready-made integrations with popular hosting platforms, including AWS, Google Cloud, Azure, Vercel, Netlify, and more. You can read more about deploying Qwik applications [here](https://qwik.dev/docs/deployments/).
